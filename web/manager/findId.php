...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {    
    ...
    $checkQuery = $db->query("SELECT * FROM WEB_USER WHERE WU_PhoneNumber = '$phoneNumber'");
    if ($checkQuery->num_rows > 0){        
        $webUserEmail;
        while($row = $checkQuery->fetch_assoc()) {
            $webUserEmail = $row['WU_Email'];
        }
        ...
        $postData = array(
            ...
            'messages' => array(
                array(                    
                    'content' => "[Pianissimo 통합관리시스템 아이디 찾기]\n 아이디: $webUserEmail", // 실제로 사용자가 수신하는 메시지 내용(정의하지 않으면 기본 메시지 컨텐츠 내용을 사용)
                    'to' => "$phoneNumber"
                    ))
        );
        ...
        if ($response_parsed->statusCode === '202') {
            echo sendResponse('findId success!', $webUserEmail, 200);
        }
        else {
