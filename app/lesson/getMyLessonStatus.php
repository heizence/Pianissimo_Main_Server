        ...
        if (preg_match('/Bearer\s(\S+)/', $authorization, $matches)) {
            ...
            if ($tokenData) { 
                ...
                if ($selectQuery->num_rows > 0){        
                    $myLessonStatus = array();
                    while($row = $selectQuery->fetch_assoc()) {
                        /* 
                        레슨 예약 날짜가 오늘이고 레슨 시작 시간이 조회하는 시점의 시각보다 빠른 시간이면 조회 대상에서 제외.
                        예) 오늘 날짜와 시간 : 2023년 2월 20일 20시
                        레슨 데이터 1 : 2023년 2월 20일 19시
                        레슨 데이터 2 : 2023년 2월 20일 22시
                        3가지 데이터 모두 날짜는 똑같음. 레슨 데이터 1의 경우 시간이 조회 시점의 시간보다 빠르므로 조회 대상에서 제외,
                        레슨 데이터 2의 경우 조회 시점의 시간보다 늦으므로 조회 대상에 포함
                        */
                        ...
                        $eachData['startTime'] = (int)$row['LR_StartTime'];
                        array_push($myLessonStatus, $eachData);
                    }
                 
                    $totalCount = sizeof($myLessonStatus);  // 조건에 해당되는 레슨 데이터 최종 갯수
                    ...
                    */
                    ...
                    $resData['myLessonStatus'] = $myLessonStatus;
